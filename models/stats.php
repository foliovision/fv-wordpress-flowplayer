...
class FV_Player_Stats {
  ...
  public function get_top_video_watch_time_stats( $range, $user_id ) {
    ...
    $results = $wpdb->get_results( "SELECT date, id_player, id_video, caption, src, SUM(seconds) AS seconds FROM `{$wpdb->prefix}fv_player_stats` AS s JOIN `{$wpdb->prefix}fv_player_videos` AS v ON s.id_video = v.id WHERE $interval $user_check GROUP BY id_video, date ORDER BY sum(seconds) DESC LIMIT 10", ARRAY_A );
