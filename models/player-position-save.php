...
class FV_Player_Position_Save {
  ...
  function player_position_save_admin_default_options_html() {
    global $fv_fp;
    $fv_fp->_get_checkbox(__('Remember video position', 'fv-wordpress-flowplayer'), 'video_position_save_enable', __('Stores the last video play position for users, so they can continue watching from where they left.'), __('It stores in usermeta database table for logged in users and in a localStorage or cookie for guest users.'));
  }
  ...
  function shortcode( $attributes, $media, $fv_fp ) {
    ...
    if ( $fv_fp->_get_option('video_position_save_enable') ) {
      ...
      if( $player_id ) { // add id to data item if db player
        ...
        if ( $metaItem = get_user_meta( get_current_user_id(), 'fv_wp_flowplayer_player_playlist_' . $player_id, true ) ) {
