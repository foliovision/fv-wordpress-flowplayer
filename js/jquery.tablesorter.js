...
;( function( $ ) {
  ...
  // Custom file sizes parser. Removes file size notations (Kb, Mb, Gb...)
  // and converts the result into a bytes-sized number.
  $.tablesorter.addParser({
    ...
    format: function(s) {
      // check if we don't have a numeric value already
      if (parseInt(s) == s) {
        return s;
      }
      ...
      switch (modifier) {
        case 'bytes': return arr[0];
          break;

        case 'KB':    return parseInt(arr[0]) * 100000;
          break;

        case 'MB':    return parseInt(arr[0]) * 1000000;
          break;

        case 'GB':    return parseInt(arr[0]) * 1000000000;
          break;

        default:      return 0; // unknown modifier means unknown filesize
      }
    },
    type: 'numeric'
  });
  ...
  $.tablesorter.addParser({
    ...
    format: function(s) {
      ...
      if ((m = regex.exec(s)) !== null) {
        ...
        return new Date(m[3], month, m[2]).getTime() / 1000;
      }
