...
=======
    // Update selector based on whether we're in standalone mode or media browser mode
    var selector = options.standalone_mode ? 
      '#' + options.container_id + ' .' + input_class_name :
      '.media-modal .media-frame-toolbar .media-toolbar-secondary > #' + upload_button_class + '-wrap .' + input_class_name;
    
    $uploadInput = $(selector);
    $uploadInput.change(function() {
>>>>>>> 9b333dfebf587ffb6673b0cdac84806c180c465e
      if( wizard_callback ) {
  ...
  function upload( file ) {
    ...
    // Only set upload status if we're in media browser mode
    if (!options.standalone_mode && typeof fv_player_media_browser !== 'undefined') {
  ...
  // Function to create upload interface
...
>>>>>>> 9b333dfebf587ffb6673b0cdac84806c180c465e
  ...
  // Function to initialize uploader elements
  function initUploaderElements($container) {
    var $uploadDiv = $container;
    
    // check if we have the correct player version (only in media browser mode)
    if (!options.standalone_mode && !fv_player_coconut_dos_upload_settings.can_use_get_space) {
      $uploadDiv.append('<button type="button" class="button media-button button-primary button-large ' + upload_button_class + '">' + options.upload_button_text + '</button>');
    ...
    var upload_interface = createUploadInterface();
  ...
  // If standalone mode, initialize immediately
  if (options.standalone_mode && options.container_id) {
    var $container = $('#' + options.container_id);
    if ($container.length) {
      initUploaderElements($container);
    }
  } else {
    // Original media browser mode
