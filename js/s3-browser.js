...
function fv_flowplayer_media_browser_setColumns() {
  var  width = jQuery('#__assets_browser').width(); // from WP

  if ( width ) {
    var columns = Math.min( Math.round( width / fv_flowplayer_idealColumnWidth ), 12 ) || 1;
    jQuery('#__assets_browser').closest( '.media-frame-content' ).attr( 'data-columns', columns );
...
jQuery( function($) {
    var
      ...
      tabId = 'fv_flowplayer_s3_browser_media_tab';

    function isCurrentBrowserTabActive() {
      return jQuery('#' + tabId).hasClass('active');
    }
  ...
  $( document ).on( "mediaBrowserOpen", function(event) {
    fv_flowplayer_media_browser_add_tab(tabId, 'Amazon S3', fv_flowplayer_s3_browser_load_assets);
  });
  ...
  fv_flowplayer_browser_browse = function(data, options) {
