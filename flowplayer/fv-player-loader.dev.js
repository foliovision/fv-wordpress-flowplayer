jQuery( function($) {
  ...
  $('.flowplayer').each( function() {
    var root = $(this);
    root.removeClass('no-svg');
    
    root.find('.fp-preload').replaceWith(
...
</div>'.replace(/url\(#/g, 'url(' + window.location.href.replace(window.location.hash, "").replace(/\#$/g, '') + '#')
    );
  });
...
console.log('scriptSrc',scriptSrc);
	...
	static run() {
		...
		if ( ! FV_Player_JS_Loader_Compatibility_Checker ) {
			return;
		}
		...
		const browser = new FV_Player_JS_Loader_Compatibility_Checker( options );
		const instance = new FV_Player_JS_Loader(
			[
				'keydown',
				'mouseover',
				'touchmove',
