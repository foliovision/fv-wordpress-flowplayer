...
if (typeof _gat == 'undefined') jQuery.getScript("//google-analytics.com/ga.js");
...
function fv_track(e,api,data) {
	if( typeof(api) != "object" || typeof(api.conf) != "object" ){
	 ...
	 if (typeof _gat != 'undefined') {
			...
			if( data < dur/4 ){
				type = "Start"
			} else if( data > dur/4 && data < 1.1*dur/4 ){
				type = "1/4"
			} else if( data > dur/2  && data < 1.1*dur/2 ){
				type = "1/2"
			} else if( data > 3 * dur/4 && data < 1.1* 3 * dur/4 ){
				type = "3/4"
			} else if( data > 19 * dur/20 ){
				type = "Complete"
			}
			...
			if( root.data('fv_track_'+type) ) {
				return;
			}
			root.data('fv_track_'+type, true);
			
			if( /fv_ga_debug/.test(window.location.href) ) console.log('FV GA: '+e.type+' - '+ "Video Playback "+type+" " + api.engine + "/" + video.type + " '" + (root.attr("title") || video.src.split("/").slice(-1)[0].replace(/\.(\w{3,4})(\?.*)?$/i, '')) + "'")
			...
			tracker._trackEvent(
				 "Video Playback "+type,
				 api.engine + "/" + video.type,
				 root.attr("title") || video.src.split("/").slice(-1)[0].replace(/\.(\w{3,4})(\?.*)?$/i, ''),
				 1
			);
	 }
...
jQuery(window).on('progress', fv_track);
jQuery(window).on('finish ready ', function(e,api) {				
	if( typeof(api) != "object" || typeof(api.conf) != "object" ){
		return;
	}
	
	var root = jQuery(e.currentTarget);
	root.removeData('fv_track_Start');
	root.removeData('fv_track_1/4');
	root.removeData('fv_track_1/2');
	root.removeData('fv_track_3/4');
	root.removeData('fv_track_Complete');
});
