...
flowplayer( function(api,root) {
  ...
  function dash_quality(api) {
    ...
    var bitrates = mediaPlayer.getTracksForTypeFromManifest( 'video', data, mediaPlayer.getStreamsFromManifest(data)[0] )[0].bitrateList;
      jQuery(bitrates).each( function(k,v) {
        console.log('bitrate',k,v);
      });
    });
  }
  
  api.bind('ready', function(e,api) {
    if(api.engine.engineName == 'dash' ) {console.log('dash tweaks!');
      /*api.engine.dash.setAutoSwitchQuality(false);
  ...
  //  todo: find a way of setting mediaPlayer.setAutoSwitchQuality(false); before it starts playing
});
