...
if( typeof(fv_flowplayer_conf) != "undefined" ) {
  ...
  if( !window.navigator.userAgent.match(/Chrome\/5[5-9]/) && ( isOldAndroid || iOSVersion < 7 ) && ('ontouchstart' in window) ) {
    flowplayer.conf.native_fullscreen = true;
  }
}
...
function fv_autoplay_init(root, scrollOffset , index ,time){
  ...
  if(index){
    if( flowplayer.support.firstframe ) {
      ...
      jQuery('[rel='+root.attr('id')+'] a').eq(index).addClass('is-active');
...
jQuery(window).on('resize',function(){
  var iframe = jQuery('iframe[id][src][height][width]'); 
  iframe.each(function(){
    if( jQuery(this).attr('id').match(/(fv_vimeo_)|(fv_ytplayer_)/) && jQuery(this).width() <= jQuery(this).attr('width') )
    jQuery(this).height( jQuery(this).width() * jQuery(this).attr('height') / jQuery(this).attr('width') );
  })
}).trigger('resize');
 ...
 */
flowplayer(function(api, root) {
  if( window.navigator.userAgent.match(/Chrome\/5[5-9]/) ) {
        ...
        setTimeout( function() {          
          if( video.src.match(/fpdl.vimeocdn.com/) && ( video.time == 0 || video.time == 1 ) ) {          
            ...
            api.on('progress', function(e,api) {
              jQuery(root).prev().find('.fv-player-warning-firefox').remove();
            });          
          }
        }, 1500 );
      });
...
/*
 *  Subtitles .is-wide class
 */
flowplayer(function(api, root) {
  root = jQuery(root);
  var checker = false;
  api.on('ready resume',function(){
...
flowplayer(function(api, root) {
  ...
  api.on('load',function(e,api,video) {
    if( video.type.match(/audio\//) ) {
      ...
      jQuery(root).addClass('is-mouseover');
      jQuery(root).addClass('is-audio');
...
flowplayer( function(api,root) {
  ...
  var cues_shown = false;
  
  api.bind('cuepoint', function(e,api,cuepoint) {
    if( cuepoint.popup) {
      var popup = root.find('.wpfp_custom_popup[data-popup-id='+cuepoint.popup+']');
      var popup_content = popup.find('.wpfp_custom_popup_content');      
      popup_content.html(popup_content.data('content'));
      popup.show();
      
      //  timeout action
      if( cuepoint.duration > 0 ) {
        setTimeout( function() { popup.hide(); }, cuepoint.duration*1000 );
      } else {
        api.pause();
  ...
  root.on('ready mouseover', function(e) {
    show_cues();
  });
  
  function show_cues() {    
    if( !root.hasClass('is-mouseover') || cues_shown ) return;
    
    var timeline = root.find('.fp-timeline:visible');
    if( timeline.length == 0 ) return;
    
    var controls = root.find('.fp-controls');
    
    jQuery(api.cuepoints).each( function(k,v) {
      var marker = jQuery('<div class="fp-timeline-tooltip fp-tooltip fv-ta-marker">'+v.popup+'</div>');      
      marker.appendTo(controls);
      
      marker.css('left', ( timeline.css('margin-left').replace(/px/,'') - marker.outerWidth()/2 +  timeline.width()/api.video.duration * v.time  ) +'px');
    });
...
( function($) {
  var dragging = false;
  var current_cue_id = false;
  var timeline;
  
  $(document).on('mousedown', '.fv-ta-marker', function(e) {    
    dragging = jQuery(e.target);
    console.log('drag started',dragging.offset().left);
    ...
    if( !marker.hasClass('fv-ta-maker-end') ) {
      var pos = jQuery('.fv-ta-marker').index(marker);
      var api = marker.parents('.flowplayer').data('flowplayer');
      var controls = marker.parents('.flowplayer').find('.fp-controls');
      timeline = marker.parents('.flowplayer').find('.fp-timeline');
      
      if( api.cuepoints && api.cuepoints[pos] ) {
        jQuery('.fv-ta-maker-end').remove();
        current_cue_id = pos;
        var current_cue = api.cuepoints[pos];
        var duration_marker = jQuery('<div class="fp-timeline-tooltip fp-tooltip fv-ta-marker fv-ta-maker-end"></div>');
        duration_marker.appendTo(controls);
        duration_marker.css('left', ( timeline.css('margin-left').replace(/px/,'') - marker.outerWidth()/2 + timeline.width()/api.video.duration * ( parseInt(current_cue.time) + parseInt(current_cue.duration) ) ) +'px');
        console.log(current_cue,( parseInt(current_cue.time) + parseInt(current_cue.duration) ));
      }
    }
    
    console.log('current_cue_id',current_cue_id);
    var input = jQuery( marker.hasClass('fv-ta-maker-end') ? '.fv_player_action_time_end' : '.fv_player_action_time',window.parent.document).eq(current_cue_id);
    input.val('changed!');
  ...
  $(document).on('mouseup', function(e) {console.log('drag end');
    if( dragging ) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  $(document).on('mousemove', function(e) {
    ...
    if( dragging ) {
      var new_x = e.pageX - timeline.css('margin-left').replace(/px/,'') - dragging.outerWidth()/2;
      /*if( e.pageX < timeline.offset().left ) {
        new_x = timeline.offset().left - shift;
      }*/
      dragging.css('left', new_x + 'px' );  
      console.log('drag in progress',timeline.css('margin-left').replace(/px/,'') - dragging.outerWidth()/2,e.pageX);
    }
  });
})(jQuery);