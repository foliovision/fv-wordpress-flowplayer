/*!
   ...
   Flowplayer Unlimited v5.4.4 (2013-11-06) | flowplayer.org/license
...
jQuery.fn.fv_fp_overlay = function() {
var ACTIVE = "is-active";

  function toggle(el) {
    jQuery("body").toggleClass("is-overlayed", !!el);
    
    if (el) {
      el.addClass(ACTIVE).trigger("open");
      jQuery(el).find(".flowplayer").attr('style-tmp', jQuery(el).find(".flowplayer").attr('style') );         
      jQuery(el).find(".flowplayer").attr('style', '');         
    } else {
      els.filter("." + ACTIVE).removeClass(ACTIVE).trigger("close");
      jQuery(els).each( function() {
        if( jQuery(this).find(".flowplayer").attr('style-tmp') ) {
          jQuery(this).find(".flowplayer").attr('style', jQuery(this).find(".flowplayer").attr('style-tmp') );      
        }
      } );
    }
  }
  ...
  // close
  jQuery(".close", this).click(function (e) {
    toggle();
    e.stopPropagation();
  });
...
jQuery(function () {
  if (flowplayer.support.inlineVideo) {
    jQuery(".fv_fp_overlay").fv_fp_overlay().bind("close", function() {
      jQuery(this).find(".flowplayer").data("flowplayer").unload();  
    });
  }
});
