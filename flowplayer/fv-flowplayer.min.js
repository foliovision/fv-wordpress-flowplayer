    ...
    if(  navigator.userAgent.match(/iPhone.* OS [0-6]_/i)){
      api.one('progress', function(e) {
        if( typeof(api.video.subtitles) !== 'undefined' && api.video.subtitles.length ){
          jQuery('#' + jQuery(root).attr('id').replace(/wpfp_/,'fv_player_iphone_subs_warning_')).show();
        }        
      });         
    }    
  }); 
...
if( typeof(flowplayer) != "undefined" ) { //  stop lightbox from playing if it's closed
  flowplayer(function (api,root) {
    var warningbox = jQuery(root.id.replace(/wpfp_/,'#fp_warnings_'));

    //ihone subs
    if(1|| navigator.userAgent.match(/iPhone.* OS [0-6]_/i)){
      api.one('progress', function(e) {
        if(1 || typeof(api.video.subtitles) !== 'undefined' && api.video.subtitles.length ){
          jQuery(warningbox).show().find('.fv-player-warning-iphone-subs').show();
        }        
      });         
    }
    
    //unstable andorid
    if(1|| !navigator.userAgent.match(/Chrome/i) && navigator.userAgent.match(/Android ([123].[\d](.[\d])?|4.[321](.[\d])?).*((GT|SGH|SCH|SHV)-[a-z]\d{3}|samsung)/i)){
      jQuery(warningbox).show().find('.fv-player-warning-unstable-android').show();
    }
