...
flowplayer(function(api, root) {
  ...
  if( flowplayer.conf.mobile_force_fullscreen && flowplayer.support.fvmobile && !flowplayer.support.fullscreen ) { // todo: make this work with Ajax loaded videos?
    ...
    api.on('resume', function() {
      ...
      if( api.isFakeFullscreen ) api.fakeFullscreen(true);
    }).on('unload', function(a,api) {
      api.fakeFullscreen(false);
    });
    
    api.fakeFullscreen = function( flag ) {
      if( api.isFakeFullscreen == flag || api.disabled ) return;
