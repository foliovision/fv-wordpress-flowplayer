...
function fv_autoplay_init(root, scrollOffset , index ,time){
  ...
  if(index){
    if( flowplayer.support.firstframe ) {
      ...
      if( fTime ) api.one('ready', function() { api.seek(fTime) } );
...
if (typeof (flowplayer) !== "undefined" && typeof(fv_flowplayer_conf) != "undefined"  && fv_flowplayer_conf.video_hash_links ) {
  ...
  jQuery(document).ready(fv_autoplay_exec).on('clickDISABLED','a[href*="fvp_"]', function() {
    if( jQuery(this).parents('.fvp-share-bar').length == 0 ) fv_autoplay_exec()
  });
