...
function fv_flowplayer_admin_test_media( hash, media ) {
	jQuery(document).ready( function() { 
		...
		jQuery.post( 'http://localhost/fv-services/video-check.php', { action: 'fv_wp_flowplayer_check_mimetype', media: media, hash: hash }, function( response ) {
			var obj;
			try {
        ...
        for( var i in obj ) {
          ...
          if( typeof(obj[i].info) != "undefined" && Object.keys(obj[i].info).length > 0 ) {
            sCheckerInfo += '<p>';
            for( var j in obj[i].info ) {
              sCheckerInfo += j+': <tt>'+obj[i].info[j]+'</tt><br />';
            }
            sCheckerInfo += '</p>';
          }
          ...
          if( typeof(obj[i].errors) != "undefined" && Object.keys(obj[i].errors).length > 0 ) {
            ...
            sCheckerInfo += '<p>';
            for( var j in obj[i].errors ) {
              sCheckerInfo += j+': <tt>'+obj[i].errors[j]+'</tt><br />';
            }
            sCheckerInfo += '</p>';            
          }
          if( typeof(obj[i].warnings) != "undefined" && Object.keys(obj[i].warnings).length > 0 ) {
            ...
            sCheckerInfo += '<p>';
            for( var j in obj[i].warnings ) {
              sCheckerInfo += j+': <tt>'+obj[i].warnings[j]+'</tt><br />';
            }
            sCheckerInfo += '</p>';            
          }
          jQuery('#wpfp_notice_'+hash).find('.video-checker-result').addClass(sResponseClass).html('Video OK');
