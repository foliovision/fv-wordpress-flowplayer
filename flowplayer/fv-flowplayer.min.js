...
if (typeof (flowplayer) != "undefined") {
  ...
  flowplayer(function (api, root) {
    api.on('ready',function(){
      var hash = parse_sharelink(api.video.sources[0].src);
  ...
  if(hash.length  ){
    flowplayer(function (api, root) {
      jQuery(document).ready(function () {
        var site_offset = ( jQuery('.site-inner').length ? jQuery('.site-inner').offset().top : 0 ) ; // ???
        var offset = $(root).offset().top - site_offset;
        if(typeof(api.conf.playlist) !== 'undefined' && api.conf.playlist.length > 1){
          for(item in api.conf.playlist){
            var src = parse_sharelink(api.conf.playlist[item].sources[0].src);
            if(hash === src){              
          ...
          var src = parse_sharelink(api.conf.clip.sources[0].src);
          if(hash === src){
            api.load();
            api.one('ready',function(){
              window.scrollTo(0,offset);
