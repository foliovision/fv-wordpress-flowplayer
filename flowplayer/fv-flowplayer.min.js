...
/*
 *  Subtitles option localstorage
 */
flowplayer( function(api,root) {
  root = jQuery(root);

  api.on('ready', function(e,api,video) {
    if ( jQuery(root).find('strong.fp-cc').is(":visible") && window.localStorage.fv_player_subtitle ) {
      if(api.video.subtitles.length){
        api.video.subtitles.forEach(function (item, index) {
          if(item.srclang === window.localStorage.fv_player_subtitle) {
            ...
            console.log('Loaded :' , window.localStorage.fv_player_subtitle);
          }
        });
      }
    }
