...
jQuery(document).ready( function() {
  ...
  flowplayer( function(api,root) {
    ...
    if( playlist.length ){            
      vertical_playlist_responsibilize(root,playlist);
      jQuery(window).resize(function(){
        vertical_playlist_responsibilize(root,playlist);
      })
    }
    function vertical_playlist_responsibilize(root,playlist){
      if(window.innerWidth < 600){
        playlist.hide();
        jQuery(root).parents('.fp-playlist-vertical-wrapper').removeClass('fp-playlist-vertical-wrapper').addClass('fp-playlist-vertical-wrapper-hidden');
        jQuery('.fp-prev,.fp-next',root).show();
      }else{
        playlist.show();
        jQuery(root).parents('.fp-playlist-vertical-wrapper-hidden').addClass('fp-playlist-vertical-wrapper').removeClass('fp-playlist-vertical-wrapper-hidden');
        jQuery('.fp-prev,.fp-next',root).hide();
      }
