...
if( typeof(fv_flowplayer_conf) != "undefined" ) {
  ...
  if( !flowplayer.support.android && flowplayer.conf.dacast_hlsjs ) {
    class FVAbrController {
      constructor (hls) {
        this.hls = hls;
        this._nextAutoLevel = -1;
        this.lastLoadedFragLevel = 3;
      }
...
function fv_player_preload() {
  ...
  flowplayer( function(api,root) {
    ...
    var fp_ratio_original = fp_ratio[0].style['padding-top'];
    var root_original = root.css('max-height');
    jQuery(root).on('click', '[data-subtitle-index]', function() { // todo: what if subtitles were enabled from the start
      subtitles_space( jQuery(this).data('subtitle-index') > -1 );        
    });
    
    api.on('ready', function() {
      subtitles_space( root.find('.fp-subtitle-menu .fp-selected').data('subtitle-index') > -1 );
    });
    
    function subtitles_space( on = true ) {
      if( on ) {
        root.addClass('active-captions');
        fp_ratio.css('padding-top', 'calc('+fp_ratio_original+' + 5em)');
        root.find('.fp-engine').css('height','calc(100% - 5em)');
        root.css('max-height', 'calc('+root_original+' + 5em)');
      } else {
        root.removeClass('active-captions');
        fp_ratio.css('padding-top', fp_ratio_original);
        root.find('.fp-engine').css('height','100%');
