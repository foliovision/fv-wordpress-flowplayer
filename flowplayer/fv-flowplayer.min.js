...
if( typeof(flowplayer) != "undefined" ) { //  stop lightbox from playing if it's closed
  flowplayer(function (api,root) {
    ...
    if( navigator.userAgent.match(/iPhone.* OS [0-6]_/i)){
      api.one('progress', function(e) {
        if( typeof(api.video.subtitles) !== 'undefined' && api.video.subtitles.length ){
          aWarnings.push(fv_flowplayer_translations.warning_iphone_subs);
        }        
      });         
    }
    ...
    if( !navigator.userAgent.match(/Chrome/i) && navigator.userAgent.match(/Android ([123].[\d](.[\d])?|4.[321](.[\d])?).*((GT|SGH|SCH|SHV)-[a-z]\d{3}|samsung)/i)){
      aWarnings.push(fv_flowplayer_translations.warning_unstable_android);
    }
    
    //aWarnings.push(fv_flowplayer_translations.warning_iphone_subs);
    //aWarnings.push(fv_flowplayer_translations.warning_unstable_android);
    if(aWarnings.length){
      var sWarnings = '<div id="' + root.id.replace(/wpfp_/,'#fp_warnings_') + '" class="fv-player-warning-wrapper">';
      for(i in aWarnings){
        if(!aWarnings.hasOwnProperty(i))
          continue;
        sWarnings += '<div class="fv-player-warning">' + aWarnings[i] + '</div>';
      }
      sWarnings += '</div>';
      
      jQuery(root).after(sWarnings);
    }
  }); 
}