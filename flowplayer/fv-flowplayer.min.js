...
jQuery(document).ready( function() {
  ...
  flowplayer( function(api,root) {
    ...
    if( playlist.length ){
      playlist.height(jQuery(root).height());  
      jQuery(window).resize(function(){
        playlist.height(jQuery(root).height());
      })
    }
  ...
  if( flowplayer.support.firstframe && typeof(fv_flowplayer_autoplay_array) != "undefined" ) {
...
flowplayer( function(api,root) {
  ...
  api.bind('ready', function(e,api) {
    ...
    if( /android/.test(navigator.userAgent.toLowerCase()) || typeof(api) == "undefined" || api.engine.engineName == "flash" || !flowplayer.support.inlineVideo ) {
...
flowplayer(function(player, root) {
  ...
  player.embedCode = function() {    
    ...
    var width = video.width || flowplayer.common.width(root);
    var height = video.height || flowplayer.common.height(root);
    height += 2;
    return '<iframe src="' + root.data('fv-embed') + '" allowfullscreen style="width:' + width + 'px;height:' + height + 'px;border:none;"></iframe>';
  };
 ...
 */
jQuery(document).ready(function(){
  if( typeof(jQuery().fv_player_colorbox) == "undefined" ) return;
console.log(flowplayer.lightbox);
  if( 0 && fv_player_pro.lightbox_images ) {
    jQuery(".colorbox").not('[href^="#"]').fv_player_colorbox( {
        /*className: 'fv_player_pro_lightbox',*/
        rel: 'group1',
        ...
        title: function() {
          if( typeof(jQuery(this).attr('title')) == "undefined" && typeof(jQuery(this).find('img').attr('alt')) == "undefined" ) {
            return false;
          }
          if( jQuery(this).attr('title') && jQuery(this).attr('title').length > 0 ) return jQuery(this).attr('title');
          if( jQuery(this).find('img') && jQuery(this).find('img').attr('alt') && jQuery(this).find('img').attr('alt').length > 0 ) {
    ...
    jQuery(".colorbox[href^='#']").fv_player_colorbox( {
        /*className: 'fv_player_pro_lightbox',*/
        rel: 'group1',
        current: "{current} of {total}",
        maxHeight: '100%',
  ...
  jQuery("[id ^=fv_flowplayer_][id $=_lightbox]").fv_player_colorbox(
    {
      /*className: 'fv_player_pro_lightbox',*/
      rel: 'group1',
      current: "{current} of {total}",
      ...
      initialHeight: 48,
      initialWidth: 96,
      innerWidth: fv_lightbox_flowplayer_width,
      innerHeight: 'auto',
      scrolling: false,
      onLoad: fv_lightbox_flowplayer_shutdown,
      onCleanup: fv_lightbox_flowplayer_shutdown
  ...
  jQuery(document).bind('fv_player_pro_box_complete', function(e){
    ...
    if( fv_player_lightbox_fresh && jQuery('#fv_player_pro_boxLoadedContent').find('.flowplayer').data("flowplayer") && !flowplayer.support.touch ) {
      ...
      api.load();
    } else if( fv_player_lightbox_fresh && fv_player_pro.playlist[jQuery('#fv_player_pro_boxLoadedContent').find('.flowplayer').attr('id')] ) { //  todo: check
      FV_Flowplayer_Pro.load(jQuery('#fv_player_pro_boxLoadedContent').find('.flowplayer'));
    }
...
function fv_lightbox_resizeWidth() {
  jQuery.fv_player_colorbox.resize({width:jQuery(window).width(), height:'auto'});
  ...
  jQuery.fv_player_colorbox.resize({width:jQuery(window).width(), innerHeight:jQuery('#fv_player_pro_boxLoadedContent').children().height()});
}

function fv_lightbox_resizeHeight() {
  FV_Flowplayer_Pro.log('Too small vertically');
  jQuery.fv_player_colorbox.resize({width:'auto', height:jQuery(window).height()});
  ...
  jQuery.fv_player_colorbox.resize({innerWidth:jQuery('#fv_player_pro_boxLoadedContent').children().width(), height:jQuery(window).height()});
}
    ...
    fv_lightbox_resizeTimer = setTimeout(function() {
      if (jQuery('#fv_player_pro_boxOverlay').is(':visible')) {
        ...
        if( windowWidth > max_width && windowHeight > max_height ) {
          jQuery.fv_player_colorbox.resize({innerWidth:max_width, innerHeight:max_height});
...
function fv_lightbox_flowplayer_width() {
  var elFlowplayer = jQuery( jQuery(this).attr('href') );
