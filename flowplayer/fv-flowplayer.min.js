...
flowplayer( function(api,root) {
  ...
  api.bind( events_enter, function(e,api) {
    ...
    if( alternative || e.type == 'fakefullscreen' ) {console.log('FS out of the way!');
...
flowplayer(function(api, root) {
  ...
  if( flowplayer.conf.mobile_force_fullscreen && flowplayer.support.fvmobile && !flowplayer.support.fullscreen ) { // todo: make this work with Ajax loaded videos?
    if( !flowplayer.support.fullscreen ) {
      api.bind('ready', function() {
        api.fullscreen(true);
      });
    ...
    // preparing for multiple video pausing
...
flowplayer( function(api,root) {
  ...
  if( !api.conf.fv_stats || !api.conf.fv_stats.enabled || root.data('fv_stats') == 'no' ) return; // TODO: why was this not disabling properly?
...
flowplayer( function(api,root) {
  ...
  // TODO: this does not work, cuepoints will not be generated
  api.on('ready', function(e,api,video) {
    // add annotations into the player
    if( video.annotations && video.annotations.length > 0 ) {
      root.generate_cuepoints = 'true';
      api.generate_cuepoints = 'true';
      api.cuepoints = video.annotations;
    }
  });
  ...
  api.on('cuepoint', function(e,api,cue) {
    ...
    var html;
    if( cue.type == 'txt' ) {
      html = jQuery('<a class="fvp-cue" data-cue="'+cue.index+'" target="_blank" href="'+cue.data.link+'">'+cue.data.text+'</a>');
    } else if( cue.type == 'img' ) {
      html = jQuery('<a class="fvp-cue" data-cue="'+cue.index+'" target="_blank" href="'+cue.data.link+'"><img src="'+cue.data.image+'" /></a>');
    } else if( cue.type == 'html' ) {
      html = jQuery('<div class="fvp-cue" data-cue="'+cue.index+'">' + (cue.data.link ? '<a target="_blank" href="'+cue.data.link+'">' : '') + cue.data.html + (cue.data.link ? '</a>' : '') + '</div>');
    }
    player.append(html);
  ...
  api.on('seek', function(e,api,time) {
    jQuery(api.video.cuepoints).each( function(k,v) {
      //console.log('check cue at '+v.time+'-'+ (v.duration + v.time), time >= v.time,( v.duration + v.time ) > time );
      if( time >= v.time && ( v.duration + v.time ) > time ) {
        api.trigger('cuepoint', [api, v]);
      } else {
        player.find('[data-cue='+k+']').remove();
      }
    });
