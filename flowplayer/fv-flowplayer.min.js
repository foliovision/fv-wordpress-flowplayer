...
jQuery(function($) {
  //  sticky video
  $("div.flowplayer").each(function(index) {
    var sticky = $("div.flowplayer").eq(index).data("fvsticky");
    var globalSticky = false;
    if (flowplayer.conf.sticky_video == 1 && typeof(sticky) == "undefined") {
      globalSticky = true;
    }
    if (globalSticky || sticky) {
      stickyPlayer(index, flowplayer.conf.sticky_place, flowplayer.conf.sticky_width);
    }
  });

  function stickyPlayer(index, place, width) {
    if (typeof(index) != "undefined") {
      var $window = $(window),
        $playerDiv = $("div.fp-player").eq(index),
        $flowplayerDiv = $("div.flowplayer").eq(index),
      ...
      $window
        ...
        .on("scroll", function() {
          $playerDiv.toggleClass("is-sticky-" + place,
            $window.scrollTop() > offset && $flowplayerDiv.hasClass("is-playing")
          );
        });
    }
  }
