...
function fv_player_preload() {
  ...
  flowplayer( function(api,root) {
    ...
    if( root.hasClass('fixed-subtitles') ) {
      var fp_ratio = root.find('.fp-ratio'),
        fp_ratio_original = fp_ratio[0].style['padding-top'],
        root_original = root.css('max-height'),
        height = false;
        
      subtitles_space( api.conf.clip.subtitles && api.conf.clip.subtitles.length && api.conf.clip.subtitles[0].default == true );
      
      api.on('ready fullscreenD fullscreen-exitD', function() {
        subtitles_space( subtitles_enabled() );
      });
      
      jQuery(root).on('click', '[data-subtitle-index]', function() {
        subtitles_space( jQuery(this).data('subtitle-index') > -1 );        
      });
      
      api.on('cuepoint', function() {
        var current_height = root.find('.fp-captions').height();
        if( current_height > height ) {
          height = current_height;
          subtitles_space(true);
        }
      });
      
      function subtitles_space( on ) {console.log('subtitles_space',on);
        if( on ) {
          root.addClass('fixed-subtitles-on');
          var spacing = check_height();
          fp_ratio.css('padding-top', 'calc('+fp_ratio_original+' + '+spacing+')');
          root.find('.fp-engine').attr('style','height: calc(100% - '+spacing+') !important');
          root.css('max-height', 'calc('+root_original+' + '+spacing+')');
          //root.find('.fp-captions').css('height',height+'em');
        } else {
          root.removeClass('fixed-subtitles-on');
          fp_ratio.css('padding-top', fp_ratio_original);
          root.find('.fp-engine').removeAttr('style');
          root.css('max-height',root_original);
          //root.find('.fp-captions').removeAttr('style');
        }
      }
      
      function check_height() {
        if( !height ) {
          var test = jQuery('<div class="fp-captions fp-shown"><p>Text</p><br /><p>Text</p><br /></div>');
          test.appendTo(root.find('.fp-player'));
          height = test.height();
          test.remove();
        }
        
        return '(' + height+'px + '+(api.isFullscreen ? 3 : 1.2) +'em )';
      }
      
      function subtitles_enabled() {
        return root.find('.fp-subtitle-menu .fp-selected').data('subtitle-index') > -1;
      }
    }
