...
jQuery(document).ready(function(){
  ...
  jQuery("div[id ^=fv_flowplayer_][id $=_lightbox]").click(function(){
    //jQuery('a[href=' + jQuery(this).attr('href') + ']').trigger('click')
  })
  jQuery("a[id ^=fv_flowplayer_][id $=_lightbox], .flowplayer.lightbox-starter").fv_player_pro_colorbox(
    {
      /*className: 'fv_player_pro_lightbox',*/
      rel: 'group1',
      current: "{current} of {total}",
      ...
      innerHeight: fv_lightbox_flowplayer_height,
      scrolling: false,
      onLoad: fv_lightbox_flowplayer_shutdown,
      onCleanup: fv_lightbox_flowplayer_shutdown
    }
...
function fv_lightbox_resize_get_dimensions(content){
  var dims = {};
  if(!content){
    content = jQuery('#fv_player_pro_boxLoadedContent');
  }
  
  dims.windowWidth = jQuery(window).width() - 28;
  dims.windowHeight = jQuery(window).height() - 62;
  //console.log('content',content);
  if(content.children('img').length > 0 ) {
    var img = new Image()
    img.src = content.children('img').attr('src'); 
    dims.max_width = img.width, dims.max_height = img.height;
  }else if(content.children('iframe').length > 0 ) {
     dims.max_width = jQuery(window).width() * 0.8;
     dims.max_height = jQuery(window).height() * 0.8;
  }else if(content.find('.flowplayer').length > 0 ) {
    var objFlowplayer = content.find('.flowplayer');   
    dims.max_width = parseInt(objFlowplayer.css('max-width'));
    dims.max_height = parseInt(objFlowplayer.css('max-height'));
  }
  dims.ratioWindow = dims.windowHeight/dims.windowWidth;
  dims.ratioContent = dims.max_height/dims.max_width;
  
  
  if( dims.ratioWindow < dims.ratioContent ) {
    if( dims.windowHeight <= dims.max_height ) {
      dims.height = dims.windowHeight;
      dims.width = dims.windowWidth * dims.ratioWindow;
    } else if( dims.windowWidth <= dims.max_width ) {
      dims.height = dims.windowHeight * dims.ratioWindow;
      dims.width = dims.windowWidth;
    } 
  } else {
    if( dims.windowWidth <= dims.max_width ) {
      dims.height = dims.windowHeight * dims.ratioWindow;
      dims.width = dims.windowWidth;
    } else if( dims.windowHeight <= dims.max_height ) {
      dims.height = dims.windowHeight;
      dims.width = dims.windowHidth * dims.ratioWindow;
    }  
  }
...
function fv_lightbox_resizeColorBox() {
    ...
    fv_lightbox_resizeTimer = setTimeout(function() {
      if (jQuery('#fv_player_pro_boxOverlay').is(':visible')) {
        ...
        var dims = fv_lightbox_resize_get_dimensions();
        
        if( dims.windowWidth > dims.max_width && dims.windowHeight > dims.max_height ) {
          jQuery.fv_player_pro_colorbox.resize({innerWidth:dims.max_width, innerHeight:dims.max_height});
          jQuery('#fv_player_pro_boxLoadedContent').children().css({width:dims.max_width, height:dims.max_height});
          return;
        }
...
function fv_lightbox_flowplayer_width() {  
  ...
  var dims = fv_lightbox_resize_get_dimensions(elFlowplayer.parent());
  if( dims.windowWidth > dims.max_width && dims.windowHeight > dims.max_height ) {
    return dims.max_width;
  }
...
/*
 * Calculates the initial width of the player
 */
function fv_lightbox_flowplayer_height() {  
var elFlowplayer = jQuery( jQuery(this).attr('href') );
  var dims = fv_lightbox_resize_get_dimensions(elFlowplayer.parent());
      console.log(dims)
  if( dims.windowWidth > dims.max_width && dims.windowHeight > dims.max_height ) {
    return dims.max_height;
  }
...
flowplayer(function(api, root) {
  if( window.navigator.userAgent.match(/Chrome\/55/) ) {
