...
/* sharing hashmarks + playlist autoplay*/
  ...
  flowplayer(function (api, root) {
    api.on('ready',function(){
      ...
      jQuery('.fvp-sharing>li>a',root).each(function(){
        ...
        jQuery(this).attr('href',jQuery(this).attr('href').replace(/#.*/,'') + '#' + hash);
      })
    })
  })
  
  var fv_player_link_played = false;
  flowplayer(function (api, root) {
    jQuery(document).ready(function () {
      if(window.location.hash.substring(1).length){
        ...
        if(typeof(api.conf.playlist) !== 'undefined' && api.conf.playlist.length > 1){
          for(item in api.conf.playlist){
            ...
            if(hash === src && !fv_player_link_played ){
              fv_player_link_played = true;
      ...
      // todo can be integrated into the above
      if( flowplayer.support.firstframe && typeof(fv_flowplayer_autoplay_array) != "undefined" && !fv_player_link_played  ) {
        for( var i in fv_flowplayer_autoplay_array ) {
          if( !fv_flowplayer_autoplay_array.hasOwnProperty(i) ) continue;
          var api_autoplay = jQuery('#wpfp_'+i).data('flowplayer');
          if( typeof(api_autoplay) != "undefined" ){
            api_autoplay.load();  
          }     
        }
      }
