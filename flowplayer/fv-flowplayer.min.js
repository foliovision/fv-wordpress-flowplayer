...
function fv_player_load() {
  ...
  jQuery('.flowplayer' ).each( function(i,el) {
    ...
    /*if( root.attr('data-item-lazy') && !root.data('fvlazy-armed') ) {
      root.find('.fp-preload, .fvfp_admin_error').remove();
      root.data('fvlazy-armed',true);
      root.one('mouseenter', function() {
        console.log('FV Player lazy load');
        root.flowplayer( { clip: fv_player_videos_parse(root.attr('data-item-lazy'), root) });
      })
    } else */if( root.attr('data-item') ) {
...
flowplayer(function (api,root) {
  ...
  api.bind("ready", function (e, api) {
    ...
    if (!root.hasClass("is-cva")){
      if (current > root.data('ad_show_after') ){
        var ad = root.find(".wpfp_custom_ad_content");
        ad.fadeIn();
        // check if the ad contains any video and pause the player if the video is found
        if( ad.find('video').length ) {
          api.pause();
        }
      }
    }
...
flowplayer(function(api, root) {
  ...
  function should() {
    if( flowplayer.conf.wpadmin || root.hasClass('is-audio') || root.data('fsforce') == false ) return false;
    
    var force = root.data('fsforce') == true || flowplayer.conf.mobile_force_fullscreen && ( 'ontouchstart' in window ) && flowplayer.support.fvmobile; // todo: setting
    if( !force && !playlist.hasClass('fp-playlist-season') ) return false;
    
    return true;
  }
  
  if( ( flowplayer.support.iOS.iPad || flowplayer.support.iOS.iPhone && flowplayer.support.iOS.version >= 10 ) && flowplayer.support.iOS.version < 12 ) {
    api.bind('ready', function() {
      if( should() ) {
        api.fullscreen();
      }
    ...
    root.on('click', function() {
      if( should() && !api.isFullscreen ) api.fullscreen();
    });
    
    jQuery('[rel='+root.attr('id')+'] a').on('click', function(e) {
      if( should() ) {
        if( location.href.match(/wp-admin/) && root.parents('.fv-player-editor-preview').length > 0 ) { // todo: not needed, as we check for wp-admin above?
        } else {
          if( !api.isFullscreen ) {
            api.fullscreen();
            api.resume();
  ...
  api.on('finish', function() {
    if( should() && ( api.conf.playlist.length == 0 || api.conf.playlist.length -1 == api.video.index ) ) api.fullscreen(false);
