...
flowplayer(function(api, root) {
  ...
  var is_audio = true;  //  todo: larger part of this should be decided in PHP
  jQuery(api.conf.clip).each( function(k,v) {
    if( !v.sources[0].type.match(/audio\//) ) is_audio = false;
  });
  ...
  setTimeout( function() {
    if( is_audio) {
      ...
      root.removeClass('is-splash').removeClass('is-mouseout');
      root.addClass('is-audio').addClass('fixed-controls').addClass('is-mouseover');
      root.find('.fp-ratio').remove();
      root.find('.fp-fullscreen').remove();
      root.find('.fvp-share-bar').remove();
      root.css('width', '');
      root.css('height', '');
      root.css('max-width', '');
      root.css('max-height', '');
      
      api.on('unload', function (e,api, error) {    
