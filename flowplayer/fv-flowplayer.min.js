...
flowplayer( function(api,root) {
  ...
  }).bind('error', function(e,api,error) {console.log('api.video',api);
    var video = typeof(api.video) != "undefined" && typeof(api.video.src) != "undefined" ? api.video : false;
    if( !video && typeof(api.conf.clip) != "undefined" && typeof(api.conf.clip.sources) != "undefined" && typeof(api.conf.clip.sources[0]) != "undefined" && typeof(api.conf.clip.sources[0].src) != "undefined" ) video = api.conf.clip.sources[0].src;
    
    if( typeof(video) != "string" || !video.match(/\/\/vimeo.com\/\d/) ) {
      fv_player_track(api.conf.analytics, "Video " + (root.hasClass('is-cva')?'Ad ':'') + "error", error.message, video );
    }
  });
...
function fv_player_track_name(root,video) {
  ...
  if( !name && typeof(video) == "string" ) name = video;
