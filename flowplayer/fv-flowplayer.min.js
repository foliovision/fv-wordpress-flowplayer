...
if( 1 || flowplayer.conf.mobile_force_fullscreen && ( 'ontouchstart' in window ) && flowplayer.support.fvmobile ) { // todo: setting to also enable for desktop!
  flowplayer(function(api, root) {
    ...
    if( flowplayer.support.iOS.iPad || flowplayer.support.iOS.iPhone && flowplayer.support.iOS.version >= 10 ) {
      ...
      jQuery(root).on('click', function() {
        if( /*!api.ready &&*/ !api.isFullscreen ) api.fullscreen();
      });
...
// Playlist in controlbar
// todo: global option and also shortcode arg
flowplayer( function(api,root) {
  root = jQuery(root);
