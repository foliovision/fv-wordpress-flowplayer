...
jQuery(document).ready(function(){
  ...
  if(fv_flowplayer_conf.lightbox_images ) {
    ...
    jQuery(".colorbox").filter(function() {
        return jQuery(this).data('fv-lightbox').match(/\.(png|jpg|jpeg|gif)/i);
    }).fv_player_pro_colorbox( {
  ...
  jQuery("a[id ^=fv_flowplayer_][id $=_lightbox], .flowplayer.lightbox-starter").fv_player_pro_colorbox(
    {
      /*className: 'fv_player_pro_lightbox',*/
      href:function(){return '#' + this.getAttribute('id').replace(/_lightbox/,'').replace(/fv_flowplayer_/,'wpfp_');},
      rel: 'group1',
      current: "{current} of {total}",
      inline: true,
      maxWidth: '100%',
...
function fv_lightbox_flowplayer_width() {
  var elFlowplayer = jQuery( jQuery(this).data('fv-lightbox') );
