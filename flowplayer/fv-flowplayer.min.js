...
function fv_autoplay_init(root, scrollOffset , index){
  ...
  flowplayer(function (api, root) {
    api.on('ready',function(){
      ...
      jQuery('.fvp-sharing>li>a',root).each(function(){
        jQuery(this).attr('href',jQuery(this).attr('href').replace(/%23.*/,'') + '%23' + hash);
      });
      jQuery('.sharing-link',root).attr('href',jQuery('.sharing-link',root).attr('href').replace(/#.*/,'') + '#' + hash );      
    })
  })
  ...
  jQuery(document).ready(function () {
        ...
        if(typeof(api.conf.playlist) !== 'undefined' && api.conf.playlist.length > 1){
          ...
          for(item in api.conf.playlist){
            ...
            if(hash === src && autoplay ){
              fv_autoplay_init(root, offset, parseInt(item));
              autoplay = false;
              return false;
            }
          }
          ...
          if(hash === src && autoplay){
            fv_autoplay_init(root, offset, false);
            autoplay = false;
            return false;
          };
        }
    ...
    if( autoplay && flowplayer.support.firstframe && typeof(fv_flowplayer_autoplay_array) != "undefined"  ) {
      for( var i in fv_flowplayer_autoplay_array ) {
        ...
        fv_autoplay_init(jQuery('#wpfp_'+i), false, false);     
      }
    }
  });
}