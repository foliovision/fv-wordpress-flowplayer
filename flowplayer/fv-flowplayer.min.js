...
function fv_wp_flowplayer_admin_support_mail( hash, button ) {			
	...
	if( comment_words.length == 0 || comment_text.match(/Enter your comment/) ) {
		jQuery('#wpfp_support_'+hash).before(fv_flowplayer_translations.what_is_wrong);
		jQuery('.fv_flowplayer_submit_error').fadeIn();
		return false;
	}

	if( comment_words.length < 7 ) {
		jQuery('#wpfp_support_'+hash).before(fv_flowplayer_translations.full_sentence);
		jQuery('.fv_flowplayer_submit_error').fadeIn();					
		return false;
	}
...
function fv_flowplayer_mobile_switch(id) {
	...
	if(
		...
		if( fv_fp_mobile ) {
			jQuery('#wpfp_'+id).after(fv_flowplayer_translations.mobile_browser_detected_1+document.URL+fv_flowplayer_translations.mobile_browser_detected_2);
		}
	}
}
  ...
  api.bind("load", function (e,api,data) {
    ...
    if( player.data('live') ){
      var live_check = setTimeout( function() {
        player.find('.fp-ui').append(fv_flowplayer_translations.live_stream_failed);
