...
flowplayer( function(api,root) {
    // variables
    var
        // the original FW player DIV wrapper from which to move the video
        // and where to return the video back again
        $root = jQuery(root),
        // document element converted into jQuery Object
        $doc = jQuery(document),
        // body element converted into jQuery Object
        $body = jQuery(document.body),
        // window element converted into jQuery Object
        $win = jQuery(window),

        // a DIV element that is wrapped around the $root element,
        // so when we apply an absolute position to the $root element
        // and make it sticky, the page doesn't jump up and down, since
        // this wrapper will retain the size of the original $root element
        $wrapper = null,

        // TOP position of the original container DIV for the video
        rootTop = -1,
        // RIGHT position of the original container DIV for the video
        rootRight = -1,

        // calculated from the root DIV's height
        stickyDivHeight = -1,
        stickyDivWidth = -1,
        // visual margins
        stickySideMargin = 10,
        stickyBottomMargin = 15, // allow +5px for player's progress indicator

        // whether the sticky DIV is showing on page (with the video inside)
        stickyActive = false,
        // whether the video should go to sticky position after we scrolled down enough
        shouldGoSticky = false,
        // maximum scroll position we can reach before the video moves to a sticky position
        maxScrollBeforeGoingSticky = -1,
        // display mode
        // TODO: this should come from plugin preferences
        stickyDivPositionMode = 'bottomright',

        // ms before we fade and hide the finished video - just like Facebook does
        onFinishedHideTimeout = 2000,

    // methods
    resizeStickyDiv = function() {
        // make maximum sticky DIV height 25% of screen height,
        // which is about what Facebook and YouTube (mobile) do
        stickyDivHeight = Math.round($win.height() / 4);

        // calculate proportional width from the video container itself
        stickyDivWidth = Math.round(((stickyDivHeight * $wrapper.width()) / $wrapper.height()));

        // adjust sticky DIV with the newly-calculated dimensions
        $root.css({
    ...
    // api event handlers
    videoOnPlay = function (e, api) {
        // check whether we're not showing the video in a sticky DIV already
        if (!stickyActive) {
            // calculate top and right of the original $root DIV container
            rootTop   = $wrapper.offset().top;
            rootRight = $win.width() - ($wrapper.width() + $wrapper.offset().left);

            // TODO: this should come from plugin preferences
            stickyDivPositionMode = 'bottomright';

            // calculate maximum top offset from which we should move video
            // to the sticky DIV (include half of the video height, so we don't
            // try to hide the sticky when we reach the bottom of the original DIV,
            // since that wouldn't make that big playing video visible again, only like
            // 10px of it at best)
            maxScrollBeforeGoingSticky = rootTop + Math.round($wrapper.height() / 2);

            // activate the possibility to make this video sticky on page scroll
            shouldGoSticky = true;
        }
    },

    videoOnStop = function (e, api) {
        // don't worry about this if we're not in sticky mode already
        if (stickyActive) {
            // check whether we've finished the video
            // or it's just been paused
            if (e.type === 'finish') {
                // video has finished, we can set a timer
                // to return the original video into place
                setTimeout(function () {
                    deactivateSticky();
                    rootTop = -1;
                    rootRight = -1;
                    ...
                    stickyActive = false;
                    shouldGoSticky = false;
                }, onFinishedHideTimeout);
            }
        } else {
            // even if the video has been paused/stopped, we should remove the possibility
            // of it to go sticky, since it's stopped and nobody cares
            // (we also do this for a finished video, of course)
            shouldGoSticky = false;
        }
    },

    fixStickyDivPosition = function(mode) {
        switch (mode) {
            case 'bottomright' :
                $root.css({
                    position: 'absolute',
                    top: (
                        // current windows scroll top position
                        ($win.scrollTop() + $win.height()) -
                        rootTop -
                        // height of the sticky video DIV
                        stickyDivHeight -
                        // bottom margin for the sticky DIV
                        stickyBottomMargin
                    ) + 'px',
                    right : -rootRight + stickySideMargin + 'px',
                    left : 'auto'
                });
                break;
            ...
            case 'bottomleft' :
                $root.css({
                    ...
                    left: -rootRight + stickySideMargin + 'px',
                    right: 'auto'
                });
                break;
        }
    ...
    // scroll handling (sticky DIV show/hide method)
    checkSticky = function() {
        // check whether we're playing a video at all
        if (shouldGoSticky) {
            // no need to change sticky DIV's position if we can't even see it
            if (stickyActive) {
                // move sticky div to be always where it's supposed to be,
                // making it a "sticky" CSS representation which would work
                // without noticeable performance penalty even on mobile browsers
                fixStickyDivPosition(stickyDivPositionMode);
            }

            // check if we should move the video to a sticky DIV yet...
            if (!stickyActive && $win.scrollTop() >= maxScrollBeforeGoingSticky) {
                activateSticky();
            } else if (stickyActive && $win.scrollTop() <= maxScrollBeforeGoingSticky) {
                // ... or move it out of the sticky DIV
                deactivateSticky();
            }
        }
    },
    ...
    activateSticky = function() {
        ...
        // make sure the page doesn't jump when we move the player
        $wrapper.css({
            'min-width': width,
            'width' : width,
            'min-height': height,
    ...
    // play/resume events
    api.bind('ready', videoOnPlay);
    api.bind('resume', videoOnPlay);

    // pause/stop events
    api.bind('pause', videoOnStop);
    api.bind('stop', videoOnStop);
    api.bind('finish', videoOnStop);

    // first load event, used to create a wrapper element
    api.bind('load', function() {
        $root.wrap('<div class="fp-sticky-wrapper" style="position: relative"></div>');
        $wrapper = $root.parent('.fp-sticky-wrapper');
    });

    $win.scroll( checkSticky );

    // when the browser resizes, we need to adjust position and size
    // (position because if we open DEV tools, we loose the video under it)
    $win.resize( function() {
        if (stickyActive) {
            resizeStickyDiv();
            fixStickyDivPosition(stickyDivPositionMode);
 ...
 */
function sendVideoPositions(async, callback) {
    if (async !== true) {
        async = false;
    }
        ...
        // methods
            ...
            storeVideoPosition = function (e, api) {
                if (api.video.sources) {
                    ...
                    var
                        originalVideoApiPath = (typeof(api.video.sources_original) != "undefined" && typeof(api.video.sources_original[0]) != "undefined" ? api.video.sources_original[0] : api.video.sources[0]),
            ...
            removeVideoPosition = function (e, api) {
                if (api.video.sources) {
                    ...
                    delete(flowplayer['playPositions'][fv_parse_sharelink(api.video.sources[0].src)]);
                }
            },
