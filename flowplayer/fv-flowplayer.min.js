...
jQuery(document).ready( function() {
  ...
  if( flowplayer.support.touch ) {
    jQuery('.fp-playlist-external').addClass('visible-captions');
  }
...
function fv_player_playlist_active(playlist,item) {
  if(playlist) {
    ...
    jQuery('.now-playing',playlist).remove();
  }
  jQuery(item).addClass('is-active');
  if( jQuery(item).find('.now-playing').length == 0 ) jQuery(item).prepend('<strong class="now-playing"><span>'+fv_flowplayer_translations.playlist_current+'</span></strong>');
}
