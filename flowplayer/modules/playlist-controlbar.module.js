...
flowplayer( function(api,root) {
  ...
  api.on('ready', function(e,api,video) {
    ...
    var thumb = playlist_menu.find('a[data-index='+api.get_video_index()+']');
    thumb.addClass('fp-selected');
    var label = fv_flowplayer_translations.playlist_item_no
    label = label.replace( /%d/, item_index.indexOf(api.get_video_index()) + 1 );
    label = label.replace( /%s/, parse_title( thumb.find('h4') ) );
    playlist_button.html(label);
    playlist_button_name.html( (item_index.indexOf(api.get_video_index()) + 1) + '. ' + track_item[api.get_video_index()] );
  });
