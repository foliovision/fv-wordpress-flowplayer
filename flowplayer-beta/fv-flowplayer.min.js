...
flowplayer( function(api,root) {  
  ...
  if( document.location.search.match(/dash_debug/) ) var debug_log = jQuery('<div class="fv-debug" style="background: gray; color: white; top: 10%; position: absolute; z-index: 1000">').appendTo(root.find('.fp-player'));
  api.bind('progress', function(e,api) {
    if(api.engine.engineName == 'dash' ) {
      var stream_info = bitrates[api.engine.dash.getQualityFor('video')];
      if( stream_info.qualityIndex != quality ) {
        quality = stream_info.qualityIndex;
        root.find('a[data-quality]').removeClass('is-current');
        root.find('a[data-quality='+stream_info.qualityIndex+']').addClass('is-current');
        
        var label = 'LD';
        if( stream_info.height >= 400 ) label = 'SD';
        if( stream_info.height >= 720 ) label = 'HD';
        if( stream_info.height >= 1280 ) label = 'FHD';
        if( stream_info.height >= 1400 ) label = 'QHD';
        if( stream_info.height >= 2000 ) label = '4K';
        root.find('.fp-qsel').html(label);
      }
      if( document.location.search.match(/dash_debug/) ) debug_log.html( "Using "+stream_info.width+"x"+stream_info.height+" at "+Math.round(stream_info.bitrate/1024)+" kbps" );
    }
  });
  
  if( document.location.search.match(/hls_debug/) ) {
