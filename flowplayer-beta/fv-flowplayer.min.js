...
flowplayer( function(api,root) {
  ...
  api.on('load', function(e,api,video) { // if the m3u8 doesn't load properly...
    if( api.engine.engineName == 'html5' && is_worthwhile(video) ) {
      console.log('HLS playback saver on load event checking...');
      
      no_progress = setTimeout( function() {
        if( !api.ready && !api.playing && api.loading && !api.finished ) {
          console.log('HLS playback saver detected error during load!');
          api.trigger('error', [api, { code: 4, video: video }]);
        }
      }, 5000 );
  ...
  api.on('ready', function() {
    ...
    if( api.engine.engineName == 'html5' && is_worthwhile(api.video) ) {
      console.log('HLS playback saver on ready event checking...');
      
      no_progress = setTimeout( hls_check, 5000 );
      
      api.on('progress', function(e,api,time) { // we keep on checking if the video progresses. Strangely iOS doesn't bother with 404 errors, it acts as if the video plays, even the video time advances. But we deal with 403 errors here!
        clearInterval(no_progress);
        no_progress = setTimeout( hls_check, 5000 );
      });
    }
  ...
  function hls_check() {
    // if you hook this to load, then only loading will be true
    if( api.ready && api.playing && !api.loading && !api.finished ) {
      console.log('HLS playback saver detected error during playback!');
      api.trigger('error', [api, { code: 4, video: api.video }]);
    }
  }
  
  function is_worthwhile( video ) {
    return video.type == 'application/x-mpegurl' || video.type == 'video/fv-mp4' && video.src.match(/vimeo/);
  }
