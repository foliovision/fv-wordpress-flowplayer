...
flowplayer( function(api,root) {  
  ...
  api.on('ready', function() {
    if( api.engine.engineName == 'html5' && ( api.video.type == 'application/x-mpegurl' || api.video.type == 'video/fv-mp4' && api.video.src.match(/vimeo/) ) ) {
  ...
  function hls_check() {    
    if( api.ready && api.playing && !api.loading && !api.finished && !api.seeking ) {
