  ...
  flowplayer(function (api, root) {
    if( jQuery(root).find('.sharing-link').length > 0 ) {
      api.on('progress',function(e,api){
        ...
        var hash = false;
        if( typeof(api.video.sources_sharing) != "undefined" && typeof(api.video.sources_sharing[0]) != "undefined" ) {
          hash = fv_parse_sharelink(api.video.sources_sharing[0].src);
        } else {
          hash = fv_parse_sharelink(api.video.sources[0].src);
        }
...
flowplayer( function(api,root) {
  ...
  if( root.data('button-no-picture') ) {
    var button_no_picture = jQuery('<strong class="fv-fp-no-picture">NP</strong>');
  ...
  if( root.data('button-repeat') ) {
    if( api.conf.playlist.length > 0 ) {
      var playlist_button = jQuery('<strong class="fv-fp-playlist mode-normal">&#9842;</strong>'),
          playlist_menu = jQuery('<div class="fp-menu fv-fp-playlist-menu"><strong>Playlist</strong><a data-action="repeat_playlist">Repeat Playlist</a><a data-action="shuffle_playlist">Shuffle Playlist</a><a data-action="repeat_track">Repeat Track</a><a class="fp-selected" data-action="normal">Normal</a></div>').insertAfter( root.find('.fp-controls') );
      ...
      button_track_repeat.insertAfter( root.find('.fp-controls .fp-volume') ).click( function(e) {
        ...
        if( api.video.loop ) {
          ...
          jQuery(this).removeClass('is-pressed');
        } else {
          api.video.loop = true;
          jQuery(this).addClass('is-pressed');
        }
      });
