 ...
 */
flowplayer(function (api,root) {
  // last popup reference gets saved if a popup should be shown at the end of the video but an AD follows it
  // ... in that case, we need to save this popup and show it after the AD itself
  var last_popup = null;
  root = jQuery(root);
  ...
  api.bind("ready", function (e, api) {
    ...
    console.log('aa');
    var
      should_show_popup = (typeof(api.video.index) == "undefined" || api.video.index+1 == api.conf.playlist.length),
      $popup            = root.find('.wpfp_custom_popup');

    // show either existing or a previously stored popup (see comment on "last_popup" above for more info)
    if ($popup.length || last_popup) {
      // check if we should be showing the popup of saving it for later
      if (should_show_popup) {
        // there's an existing popup, so we just show it at the end of this video
        if ($popup.length) {
          setTimeout(function() {
            $popup.on('click', function() {
              flowplayer(document.getElementById(player_id)).play();
            });
            $popup.show();
          }, 1000);
        } else {
          // we have a cached popup from a previous video, which should be shown at the end of this AD
          // ... we need to use setTimeout here, since the player will be stopped and popup removed after the "finish" event is done
          setTimeout(function() {
            root.find('.fp-player').append(last_popup);
            last_popup.show();
          }, 1000);
        }
      } else {
        // store a popup clone for later showing (see comment on "last_popup" above for more info)
        last_popup = $popup.clone();
        last_popup.on('click', function() {
          flowplayer(document.getElementById(player_id)).play();
        });
      }

    } else if (typeof(fv_flowplayer_popup) != "undefined" && typeof(fv_flowplayer_popup[player_id]) != "undefined") {
      // create a new popup and change the popup variable to point to a correct element
      $popup = jQuery('<div id="' + player_id + '_custom_popup" class="wpfp_custom_popup">' + fv_flowplayer_popup[player_id].html + '</div>');
      ...
      if (should_show_popup) {
        ...
        setTimeout(function() {
          ...
          root.find('.fp-player').append($popup);
        }, 1000);
      } else {
        // store a popup clone for later showing (see comment on "last_popup" above for more info)
        last_popup = $popup.clone();
      ...
      // no popups for this player, reset last_popup variable
      last_popup = null;
    }
