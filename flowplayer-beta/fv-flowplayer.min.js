...
function fv_autoplay_init(root, scrollOffset , index ,time){
  ...
  if(scrollOffset && !root.find('.fp-player').attr('class').match(/\bis-sticky/) ){
  ...
  if(index){
    if( fv_autoplay_can(api,parseInt(item)) ) {
...
function fv_autoplay_can( api, item ) {  
  if( item ) {
    var video = api.conf.playlist[item];
  } else {
    var video = api.conf.clip;
  }
