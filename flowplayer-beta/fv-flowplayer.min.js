...
flowplayer(function(api, root) {
  ...
  function check_size() {
    ...
    if(el.width() <= 320) {
      el.addClass('no-volume fp-mute');
    } else {
      if( !had_no_volume ) el.removeClass('no-volume');
      if( !had_fp_mute ) el.removeClass('fp-mute');
    }    
  }
...
/*
 *  MPEG-DASH and HLS.js - debug for the ABR
 */
if( document.location.search.match(/dash_debug/) || document.location.search.match(/hls_debug/) ) {
  flowplayer( function(api,root) {  
    root = jQuery(root);
    ...
    api.on("hlsLevelSwitch", function (e, api, data) {      
      ...
      debug_log.html( "Using "+data.level+" "+level.width+"x"+level.height+" at "+Math.round(level.bitrate/1024)+" kbps" );
    });    
...
flowplayer( function(api,root) {
  ...
  jQuery(".fp-ui", root).click(function() {
    var
      $root = jQuery(root),
      start_index = $root.data('playlist_start');

    if (typeof(start_index) != 'undefined' && api.conf.playlist.length) {
      // make this really an index
      start_index--;

      // save state, so we don't play the same video over and over
      // if we already clicked on that player before
      if ($root.data('played') !== 1) {
  ...
  api.bind('ready', function() {
    // make sure to mark the player as played already,
    // so we don't start from the set playlist position
    // if we actually want to pause the playing video etc.
    jQuery(root).data('played', 1);
  });

  api.bind('unload', function() {
    // make sure to remove the player as played already,
    // so we can start from the correct playlist index,
    // if set
    jQuery(root).removeData('played');
  });
